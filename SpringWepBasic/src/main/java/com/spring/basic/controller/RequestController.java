...
//빈을 등록해놔야(객체가 생성되어 있어야) HandlerMapping이 이 클래스의 객체를 검색할 수 있다.
@Controller
...
public class RequestController {
	...
	@RequestMapping("/req") //매핑
	public String req() {
		return "request/req-ex01"; //경로지정
	}
...
//	@RequestMapping(value="/request/basic01", method = RequestMethod.POST)
	...
	//요청 URL 주소가 같더라도, 전송 방식에따라 맵핑을 다르게 하기 때문에
	//같은 주소를 사용하는것이 가능합니다.(GET -> 화면처리 POST -> 입력값 처리)
	
	
	/*
	 #스프링에서 요청과 함께 전달된 데이터를 처리하는 방식
	 1. 전통적인 jsp/setvlet 방식의 파라미터 읽기 처리 방법.
	...
	*/
	...
	/*
	 ...
	 # 커맨드 객체: 사용자의 입력을 담기 위해 설계되고, 특정 검증로직이나 비즈니스 로직을
