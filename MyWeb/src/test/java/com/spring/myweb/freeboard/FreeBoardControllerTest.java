...
//	@Test
//	@DisplayName("3번 글 상세보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 model에 담아 freeDerail.jsp로 이동시킬것이다.")
//	void testContent() {
//		// /freeboard/content -> get
//		// bno, title, writer, content, updateDate == null ? regDate(수정됨)
//		ModelAndView mv = mockMvc.perform(MockMvcRequestBuilders.get("/freeboard/content")) // 가상의 요청을 보냄
//								.param("bno", "3").andReturn().getModelAndView().getViewName();
//	}
	
	@Test
	@DisplayName("3번 글 상세보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 model에 담아 freeDetail.jsp로 이동시킬것이다.")
