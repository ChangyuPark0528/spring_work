...
<body>
	...
	<script>
		...
		function regist() {
			...
			console.log(file);
			//.을 제거한 확장자만 얻어낸 후 그것을 소문자로 일괄변경.
			file = file.slice(file.indexOf('.') + 1).toLowerCase();
			if(file !== 'jpg' && file !== 'png' && file !== 'jpeg' && file !== 'bmp') {
				alert('이미지 파일만(jpg, png, jpeg, bmp)만 등록이 가능합니다.');
				return;
			}
			...
			*/
			...
			const formData = new FormData(); //내장객체
			...
			//FormData 객체를 보낼때는 따로 headers 설정을 진행하지 않습니다.
			...
			//file input 내용을 비워주시고, 글영역 비워주시고,
		...
		$contentDiv.addEventListener('click', e => {
			...
			if(!e.target.matches('.image-inner img')
				...
				&& !e.target.matches('.link-inner #delBtn')
					...
					console.log('이벤트 대상이아닙니다.');
					return;
			}
			//다운로드 처리
			if(e.target.matches('.title #download')) {
				if(confirm('다운로드를 진행하겠습니다.')) {
