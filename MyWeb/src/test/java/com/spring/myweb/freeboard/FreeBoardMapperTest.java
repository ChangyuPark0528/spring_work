...
//				.build());
	...
	@DisplayName("글 번호가 11번인 글을 조회하면 "
			+ "글쓴이는 'kim1234'일 것이고, 글 제목은'메롱' 일 것이다.")
	void getContentTest() {
		...
		assertTrue(board.getTitle().equals("메롱"));
	}
	...
	@Test
	@DisplayName("글 번호가 1번인 글의 제목과 내용을 수정 후 다시 조회했을때 "
			+ "수정한 제목과 내용이 나와야함.")
	void updateTest() {
	...
	//글 번호가 2번인 글을 삭제한 후에 전체 목옥을 조회했을때
	//글의 개수는 11개일 것이고
	//2번 글을 조회했을 때 null이 리턴됨을 단언하세요. -> assertNull(객체)
	
	@Test
	@DisplayName("글 번호가 2번인 글을 삭제한 후에 전체 목옥을 조회했을때\r\n"
			+ "	글의 개수는 11개일 것이고\r\n"
